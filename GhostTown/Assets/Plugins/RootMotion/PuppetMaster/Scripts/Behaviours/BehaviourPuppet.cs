	...
	public partial class BehaviourPuppet : BehaviourBase {
		...
		protected override void OnFixedUpdate() {
			...
			if (state == State.Unpinned) {
				...
				if (puppetMaster.muscles[0].rigidbody == null)
				{
				}
				else
				{
			...
			if (state != State.Unpinned && !puppetMaster.isKilling) {
				...
				foreach (Muscle m in puppetMaster.muscles) {
                    ...
                    if (hasCollidedSinceGetUp && !puppetMaster.isBlending && offset > 0f && pinW <= pinWeightThreshold && offset > props.knockOutDistance * knockOutDistanceSqr * stateMlp) {
