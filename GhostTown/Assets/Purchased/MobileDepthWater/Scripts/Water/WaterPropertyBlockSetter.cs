namespace Assets.Scripts.Water
{
    ...
    public class WaterPropertyBlockSetter : MonoBehaviour
    {
        ...
        public void Awake()
        {
            //This part adds all platforms to this list
            GameObject[] platforms = GameObject.FindGameObjectsWithTag("Platform");
            Renderer[] newRenderers = new Renderer[platforms.Length + 1];
            Renderer plane = waterRenderers[0];
            newRenderers[0] = plane;
            for (int i = 1; i < newRenderers.Length; i++)
            {
                newRenderers[i] = platforms[i - 1].GetComponent<Renderer>();
            }
            waterRenderers = newRenderers;
